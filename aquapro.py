...
def exp_comprehensive_comparison(oracle, proxy, index, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname=''):
    ...
    for i in range(len(index)):
        ...
        for _ in range(repeat):
            ...
            precision, recall, RT_cost = test_CSE_RT(oracle_dist, ranks, t=t, delta=delta, prob=rt_prob, rt=rt, mode='approx_m1')
...
def exp_runningtime(oracle, proxy, index, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname=''):
    delta = 1 - sqrt(prob)
    rt_prob = prob / (1 - delta)
    CSE_PT_stats = defaultdict(list)
    CSE_RT_stats = defaultdict(list)
