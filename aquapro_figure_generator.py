...
def _plot_CSA(p, m_list, query_type, f, cost_list, success_list):
    ...
    for i in range(len(m_list)):
        ...
        roll_var.extend([np.std(cost_list[m_list[i]])] * trials)

    data = pd.DataFrame({'cost': roll_cost, 'success': roll_success, 'methods': roll_methods, 'std': roll_var})
    ...
    color = 'olivedrab'
    sns.barplot(x="methods", y="cost", data=data, color=color, ax=ax1, ci='sd')
    ax1.set_ylabel('#oracle calls', color=color)  # we already handled the x-label with ax1
    width_scale = 0.5

    line_shift = None
    for bar in ax1.containers[0]:
        line_shift = bar.get_width() / 4
        bar.set_width(bar.get_width() * width_scale)

    for l in ax1.lines:
        x = l.get_data()
        l.set_data([x[0]-line_shift, x[1]])

    # ax1.errorbar(data=data, x='methods', y='std', fmt='none')

    # for bar in ax1.containers[0]:
    #     x = bar.get_x()
    #     w = bar.get_width()
    #     # bar.set_x(x - 0.5 * w * (1 - width_scale))
    #     bar.set_width(w * width_scale)

    # ax1.bar_label(ax1.containers[0], fmt='%d', size=11)
    if f == 'eICU':
        ax1.set_ylim([0, 10000])  # eICU: 8500
    elif f == 'VOC':
        ax1.set_ylim([0, 6000])  # VOC: 5000
    else:
...
def _plot_CSE_PQE(p, m_l, query_type, f, pqe, cse, supg, sample2test):
    sns.set_context("talk")
    roll_methods = list()
    roll_success = list()
    roll_cr = list()

    trials = len(sample2test[0]['CR'])

    roll_cr.extend(sample2test[0]['CR'])
    roll_success.extend(sample2test[0]['success'])
    roll_methods.extend(['Sample2Test'] * trials)

    padding = int(trials / len(pqe[0]['success']))
    roll_success.extend(pqe[0]['success'] * padding)
    roll_cr.extend(pqe[0]['CR'] * padding)
    roll_methods.extend(['PQE'] * trials)

    padding = int(trials / len(cse['success']))
    roll_cr.extend(cse['CR'] * padding)
    roll_success.extend(cse['success'] * padding)
    roll_methods.extend(['CSE'] * len(cse['CR']))

    padding = int(trials / len(supg[0]['success']))
    roll_cr.extend(supg[0]['CR'] * padding)
    roll_success.extend(supg[0]['success'] * padding)
...
def _plot_COMP(prob, m_l, query_type, f, s_l, pqe, cse, supg, sample2test):
    sns.set_context("talk")
    roll_success = list()
    roll_cr = list()
    roll_methods = list()
    ...
    print('%s-%s, sample2test -50%% success %.2f, 50%% success %.2f' % (f, query_type,
                                                                        float(np.mean(sample2test[s_l[0]]['success'])),
                                                                        float(
                                                                            np.mean(sample2test[s_l[-1]]['success']))))
    print('%s-%s, sample2test -50%% CR %.2f, 50%% CR %.2f' % (f, query_type,
                                                              float(np.mean(sample2test[s_l[0]]['CR'])),
                                                              float(np.mean(sample2test[s_l[-1]]['CR']))))
    for i in range(len(s_l)):
        trials = len(sample2test[s_l[i]]['CR'])
        padding = int(trials / len(pqe[s_l[i]]['success']))
        roll_success.extend(pqe[s_l[i]]['success'] * padding)
        roll_cr.extend(pqe[s_l[i]]['CR'] * padding)
        roll_methods.extend(['PQE'] * trials)
        roll_indices.extend([x_axis[i]] * trials)
        roll_scales.extend([s_l[i]] * trials)

        padding = int(trials / len(supg[s_l[i]]['success']))
        roll_cr.extend(supg[s_l[i]]['CR'] * padding)
        roll_success.extend(supg[s_l[i]]['success'] * padding)
        roll_methods.extend(['SUPG'] * trials)
        roll_indices.extend([x_axis[i]] * trials)
        roll_scales.extend([s_l[i]] * trials)

        roll_cr.extend(sample2test[s_l[i]]['CR'])
        roll_success.extend(sample2test[s_l[i]]['success'])
        roll_methods.extend(['Sample2Test'] * trials)
        roll_indices.extend([x_axis[i]] * trials)
        roll_scales.extend([s_l[i]] * trials)

        print('scale %d, CR[PQE]-CR[SUPG] %.2f' % (
        s_l[i], float(np.mean(pqe[s_l[i]]['CR']) - np.mean(supg[s_l[i]]['CR']))))
    ...
    ax1.axhline(y=np.mean(cse['CR']), color='k', linestyle=(0, (3, 1, 1, 1, 1, 1)), label='CSE')
    ax1.legend()
    ax1.axhline(y=np.mean(cse['CR']), color=color, linestyle=(0, (3, 1, 1, 1, 1, 1)))
...
def _print_running_time(query_type, f_l, path_prefix, exp_n, sample2test_exp):
    ...
    print('%s  DATASET      PQE      CSE      SUPG      Sample2Test' % query_type)
    for f in f_l:
        f_path = path_prefix + exp_n + '/' + f + '.pkl'
        sample2test_path = path_prefix + sample2test_exp + '/' + f + '.pkl'
        f = f_map(f)
        if query_type == 'PT':
            _, pqe, cse, supg, _, _, _, _ = pickle.load(open(f_path, "rb"))
            _, sample2test, _, _ = pickle.load(open(sample2test_path, "rb"))
        else:
            _, _, _, _, pqe, cse, supg, _ = pickle.load(open(f_path, "rb"))
            _, _, sample2test, _ = pickle.load(open(sample2test_path, "rb"))
        ...
        sample2test_cost = data_size[f] * proxy_price[f] + np.mean(sample2test['cost']) * oracle_price[f] + np.mean(
            sample2test['overhead'])
        print('    %s      %.2f      %.2f      %.2f         %.2f' % (
        f, pqe_cost / 3600, cse_cost / 3600, supg_cost / 3600, sample2test_cost / 3600))
    ...
    for f in f_l:
        ...
        if query_type == 'PT':
            ...
            _, _, _, _, pqe, cse, supg, _ = pickle.load(open(f_path, "rb"))
            _, _, sample2test, _ = pickle.load(open(sample2test_path, "rb"))
        print('    %s      %d        %d        %d       %d' % (f, int(np.mean(pqe['cost'])), int(np.mean(cse['cost'])),
                                                               int(np.mean(supg['cost'])),
                                                               int(np.mean(sample2test['cost']))))


def _plot_scalability_test(s_l, query_type, f, pqe, cse, supg, sample2test):
    sns.set_context("talk")
    roll_overhead = list()
    roll_methods = list()
    roll_indices = list()
    ...
    for i in range(len(s_l)):
        ...
        roll_overhead.extend(sample2test[s_l[i]]['overhead'])
        roll_methods.extend(['Sample2Test'] * trials)
        roll_indices.extend([x_axis[i]] * trials)
    ...
    data = pd.DataFrame({'indices': roll_indices, 'overhead': roll_overhead, 'methods': roll_methods})
    hue_order = ['PQE', 'CSE', 'SUPG', 'Sample2Test']
    fig, ax1 = plt.subplots()
    ax1.set_title('%s: (%s)' % (query_type, f))
    ...
    print('                        PQE   CSE  SUPG  Sample2Test    PQE  CSE  SUPG  Sample2Testt')
    for s in s_l:
        print(' %d       %.2f       %.2f  %.2f  %.2f %.2f    %.2f  %.2f  %.2f %.2f ' %
              (s, float(np.mean(cse[s]['cost'])),
               float(np.mean(pqe[s]['success'])), float(np.mean(cse[s]['success'])), float(np.mean(supg[s]['success'])), float(np.mean(sample2test[s]['success'])),
               float(np.mean(pqe[s]['CR'])), float(np.mean(cse[s]['CR'])), float(np.mean(supg[s]['CR'])), float(np.mean(sample2test[s]['CR'])),))
    print('')


def _compact_plot_COMP_PQA_PQE(f, f_path, h_path, mode='RT'):
    # sns.set_context("talk")
    roll_datasets = list()
    roll_types = list()
    roll_scales = list()
    ...
    s_list, pt_k_recall, pt_k_success, rt_k_precis, rt_k_success, prob = pickle.load(open(f_path, "rb"))
    method_list, scale_list, PQE_PT, CSE_PT_stats, SUPG_PT, PQE_RT, CSE_RT_stats, SUPG_RT, prob = pickle.load(
        open(h_path, "rb"))

    if mode == 'RT':
        pqe_succes = np.mean(PQE_RT[0]['success'])
        pqe_cr = np.mean(PQE_RT[0]['CR'])
    elif mode == 'PT':
        pqe_succes = np.mean(PQE_PT[0]['success'])
        pqe_cr = np.mean(PQE_PT[0]['CR'])
    ...
    def twin_lineplot(x, y, z, color, **kwargs):
        ...
        color = 'tab:blue'
        ax1.set_ylabel('CR', color=color)  # we already handled the x-label with ax1
        sns.lineplot(x=x, y=y, color=color, **kwargs, ax=ax1, ci=90, label='PQA')
        # ax1.plot([0], [np.mean(cr_list[0])], color=color, marker='*', markersize=15)
        ax1.axhline(y=pqe_cr, color=color, linestyle='dotted', label='PQE')
        # ax1.legend()
        ax1.tick_params(axis='y', labelcolor=color)
        ...
        ax2.set_ylabel('success prob.', color=color)
        ax2.axhline(y=prob, color='k', linestyle='dashdot')
        ax2.axhline(y=pqe_succes, color=color, linestyle='dotted')
        sns.lineplot(x=x, y=z, color=color, **kwargs, ax=ax2, ci=None)
        # ax2.legend()
        # ax2.plot([0], [np.mean(success_list[0])], color=color, marker='*', markersize=15)
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.set_ylim([-0.05, 1.05])
        # ax2.set_xscale('log')
        # ax2.set_xticks([0.05, 0.1, 0.2, 0.4])

    g = sns.FacetGrid(data, row='query_type', height=2.5, aspect=1.4)
    g.map(twin_lineplot, 'scales', 'CR', 'success', color='r')

    g.axes[0, 0].set_title('PQA v.s. PQE (%s)' % mode)
    g.axes[0, 0].set_xlabel(r'$\sigma$')
    plt.show()


def _compact_plot_COMP_CSC_CSE(f, f_path, h_path, mode='PT'):
    # sns.set_context("talk")
    roll_datasets = list()
    roll_types = list()
    roll_scales = list()
    ...
    if mode == 'RT':
        cse_succes = np.mean(CSE_RT_stats['success'])
        cse_cr = np.mean(CSE_RT_stats['CR'])
    elif mode == 'PT':
        cse_succes = np.mean(CSE_PT_stats['success'])
        cse_cr = np.mean(CSE_PT_stats['CR'])
    ...
    def twin_lineplot(x, y, z, color, **kwargs):
        ...
        sns.lineplot(x=x, y=y, color=color, **kwargs, ax=ax1, ci=90)
        # ax1.plot([0], [np.mean(cr_list[0])], color=color, marker='*', markersize=15)
        ax1.axhline(y=cse_cr, color=color, linestyle='dotted', label='PQE')
        ax1.tick_params(axis='y', labelcolor=color)
        ax1.set_ylim([-0.05, 1.05])
        ...
        ax2.set_ylabel('success prob.', color=color)
        ax2.axhline(y=prob, color='k', linestyle='dashdot')
        ax1.axhline(y=cse_succes, color=color, linestyle='dotted', label='PQE')
        sns.lineplot(x=x, y=z, color=color, **kwargs, ax=ax2, ci=None)
        # ax2.plot([0], [np.mean(success_list[0])], color=color, marker='*', markersize=15)
        ax2.tick_params(axis='y', labelcolor=color)
        ax2.set_ylim([-0.05, 1.05])
    ...
    g = sns.FacetGrid(data, row='query_type', height=2.3, aspect=1.4)
    g.map(twin_lineplot, 'scales', 'CR', 'success', color='r')
    if mode == 'RT':
        g.axes[0, 0].set_title('CSC v.s. CSE (RT)')
    elif mode == 'PT':
        g.axes[0, 0].set_title('CSC v.s. CSE (PT)')
    # g.axes[0, 0].set_title('CSC-RT(%s)' % f)
    # g.axes[1, 0].set_title('CSE-PT(%s)' % f)
    g.axes[0, 0].set_xlabel(r'$c$ perturbation (%)')
    plt.show()


def _print_running_time_scan2test_pp_topk(query_type, f_l):
    oracle_price = {'VOC': 20, 'COCO(small)': 20, 'MIMIC-III': 900, 'eICU': 900, 'night-street': 0.4}
    proxy_price = {'VOC': 0.05, 'COCO(small)': 0.05, 'MIMIC-III': 0.001, 'eICU': 0.001, 'night-street': 0.02}
    data_size = {'VOC': 4952, 'COCO(small)': 8000, 'MIMIC-III': 4244, 'eICU': 8236, 'night-street': 10000}  # COCO:40137

    print('%s  DATASET      Scan2Test' % query_type)
    for f in f_l:
        f = f_map(f)
        scan2test_cost = data_size[f] * oracle_price[f]
        print('    %s      %.2f' % (f, scan2test_cost / 3600))


def _plot_COMP_sample2test(prob, m_l, query_type, f, s_l, sample2test, cse):
    sns.set_context("talk")
    roll_success = list()
    roll_cr = list()
    roll_methods = list()
    ...
    print('%s-%s, supg -50%% success %.2f, 50%% success %.2f' % (f, query_type,
                                                                 float(np.mean(sample2test[s_l[0]]['success'])),
                                                                 float(np.mean(sample2test[s_l[-1]]['success']))))
    print('%s-%s, supg -50%% CR %.2f, 50%% CR %.2f' % (f, query_type,
                                                       float(np.mean(sample2test[s_l[0]]['CR'])),
                                                       float(np.mean(sample2test[s_l[-1]]['CR']))))
    for i in range(len(s_l)):
...
def _plot_COMP_topk_PQA(p, m_list, query_type, f, cost_list, success_list):
    # sns.set_style("ticks")
    sns.set_context("talk")
    roll_methods = list()
    roll_success = list()
    ...
    fig, ax1 = plt.subplots()
    ax1.set_title('PQA v.s. top-K (%s)' % (query_type))
    # ax1.set_xlabel('methods')
    color = 'olivedrab'
...
if __name__ == '__main__':
    ...
    if False:
    ...
    elif exp_name == 'COMP_PQA_PQE':
        for fname in ['voc']:
            file_path = file_path_prefix + exp_name + '/' + fname + '.pkl'
            hline_path = file_path_prefix + 'CMPR' + '/' + fname + '.pkl'
            fname = f_map(fname)
            _compact_plot_COMP_PQA_PQE(f=fname, f_path=file_path, h_path=hline_path, mode='RT')
            _compact_plot_COMP_PQA_PQE(f=fname, f_path=file_path, h_path=hline_path, mode='PT')

    elif exp_name == 'COMP_CSC_CSE':
        for fname in ['voc']:
            file_path = file_path_prefix + exp_name + '/' + fname + '.pkl'
            hline_path = file_path_prefix + 'CMPR' + '/' + fname + '.pkl'
            fname = f_map(fname)
            _compact_plot_COMP_CSC_CSE(f=fname, f_path=file_path, h_path=hline_path, mode='RT')
            _compact_plot_COMP_CSC_CSE(f=fname, f_path=file_path, h_path=hline_path, mode='PT')

    elif exp_name == 'COMP_topk_PQA':
        for fname in ['voc']:
            file_path = file_path_prefix + exp_name + '/' + fname + '.pkl'
            pqa_path = file_path_prefix + 'PQA' + '/' + fname + '.pkl'
            fname = f_map(fname)
            scale_list, _, _, _, _, topk_recall, topk_precision, topk_cost, prob, pt, rt = pickle.load(
                open(file_path, "rb"))
            topk_scale = scale_list[0]
            topk_cost = topk_cost[topk_scale] * 2
            topk_rt_success = list((np.array(topk_recall[topk_scale]) >= rt) * 1) * 2
            topk_pt_success = list((np.array(topk_precision[topk_scale]) >= pt) * 1) * 2

            s_list, pt_k_recall, pt_k_success, rt_k_precis, rt_k_success, _ = pickle.load(open(pqa_path, "rb"))
            pqa_cost = [0] * len(topk_cost)
            pqa_rt_success = rt_k_success[0]
            pqa_pt_success = pt_k_success[0]

            cost_list = {'topK': topk_cost, 'PQA': pqa_cost}
            pt_success_list = {'topK': topk_pt_success, 'PQA': pqa_pt_success}
            rt_success_list = {'topK': topk_rt_success, 'PQA': pqa_rt_success}

            _plot_COMP_topk_PQA(p=prob, m_list=['PQA', 'topK'], query_type='RT', f=fname, cost_list=cost_list,
                                success_list=rt_success_list)
            _plot_COMP_topk_PQA(p=prob, m_list=['PQA', 'topK'], query_type='PT', f=fname, cost_list=cost_list,
                                success_list=pt_success_list)

    elif exp_name == 'COMP_sample2test_CSE':
        for fname in ['voc', 'icd9_eICU']:  # 'voc', 'icd9_eICU', 'icd9_mimic', 'jackson10000.csv', 'coco8000'
            file_path = file_path_prefix + exp_name + '/' + fname + '.pkl'
            fname = f_map(fname)
            method_list, scale_list, sample2test_PT, CSE_PT_stats, sample2test_RT, CSE_RT_stats, prob = pickle.load(
                open(file_path, "rb"))
            _plot_COMP_sample2test(prob=prob, m_l=method_list, query_type='RT', f=fname, s_l=scale_list,
                                   cse=CSE_RT_stats, sample2test=sample2test_RT)
            _plot_COMP_sample2test(prob=prob, m_l=method_list, query_type='PT', f=fname, s_l=scale_list,
                                   cse=CSE_PT_stats, sample2test=sample2test_PT)

    elif exp_name == 'runningtime_scan2test_pp_topk':
        file_list = ['voc', 'coco8000', 'icd9_mimic', 'icd9_eICU', 'jackson10000.csv']
        _print_running_time_scan2test_pp_topk(query_type='RT', f_l=file_list)
        _print_running_time_scan2test_pp_topk(query_type='PT', f_l=file_list)
        ...
        for fname in ['coco']:  # 'voc', 'icd9_eICU', 'icd9_mimic', 'jackson10000.csv', 'coco8000'
            file_path = file_path_prefix + exp_name + '/' + fname + '.pkl'
            sample2test_path = file_path_prefix + 'scalability_sample2test' + '/' + fname + '.pkl'
            fname = f_map(fname)
            subset_sizes, PQE_PT_stats, CSE_PT_stats, SUPG_PT_stats, PQE_RT_stats, CSE_RT_stats, SUPG_RT_stats, prob = pickle.load(
                open(file_path, "rb"))
            _, sample2test_PT_stats, sample2test_RT_stats, _ = pickle.load(open(sample2test_path, "rb"))
            _plot_scalability_test(subset_sizes, 'RT', f=fname, pqe=PQE_RT_stats, cse=CSE_RT_stats, supg=SUPG_RT_stats, sample2test=sample2test_RT_stats)
            _plot_scalability_test(subset_sizes, 'PT', f=fname, pqe=PQE_PT_stats, cse=CSE_PT_stats, supg=SUPG_PT_stats, sample2test=sample2test_PT_stats)
